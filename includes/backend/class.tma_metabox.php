...
class TMA_MetaBox {
	...
	function smashing_post_class_meta_box($object, $box) {
		...
		<p>
			...
			$metaData = get_post_meta(get_the_ID(), Constants::$META_KEY);
			$postScores = array();
			if (isset($metaData[0])) {
	...
	private function handleSegment ($segment, $postScores) {
		...
		foreach ($segment->rules as $rule) {
			if ($rule->type === "score") {
				...
				echo "</td>";
				echo "<td>";
				echo "<input type='number' name='" . Constants::$META_KEY . "[" . $rule->name . "]' ";
				echo " min='0' step='1' ";
	...
	function smashing_save_post_class_meta($post_id, $post) {
		...
		$new_meta_value_temp = ( isset($_POST[Constants::$META_KEY]) ? filter_input(INPUT_POST, Constants::$META_KEY, FILTER_DEFAULT, FILTER_REQUIRE_ARRAY) : array() );
		$new_meta_value = \array_filter($new_meta_value_temp);

		/* Get the meta key. */
		$meta_key = Constants::$META_KEY;
